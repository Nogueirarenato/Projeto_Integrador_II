...
class Login extends Component {
    ...
    componentDidMount() {
        document.title = "Zabbix Graphics Reports"
        this.setState({erro:""})
        ...
        return (
            <section id="login">
                <div className="caixa">
                    ...
                    <img src={Logo2} alt="Logo" className="image2" />
                   ...
                   <button className="login-button" onClick = {this.acessarSite.bind(this)}  {...this.state.isLoading ? "disabled" : ""}>
                        ...
                        {this.state.isLoading ? "Carregando..." : "GitHub do Projeto"}</button>
                   
                   
                   
                    <h2 style={{color:"red", fontSize: "1.5em", margin: "3%" }}>{this.state.erro}</h2>
                </div>
                <p style={{marginRight: "3vh"}}>Projeto Integrador em Computação II</p>
                <p style={{marginRight: "10vh", marginTop: "none"}} >2022</p>
            </section>)
